...
class HomeViewCtrl : UIViewController {
    ...
    // the current ui orientation of shown, remember it is UI Orientation of shown, not the device orientation
    ...
    private func initUI() {
        let (widthReal, heightReal) = UIMgr.realScreenSize
        let xInter = UIMgr.instance.standardfontSize * 0.4
        let yInter = xInter
        let yBottomInter = UIMgr.instance.standardfontSize * 0.6
