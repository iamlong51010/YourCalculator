...
class CalculatorView : UIView {
    ...
    private class func GetXInterFromFrameHeight(_ height: CGFloat) -> CGFloat {
        let (_, heightReal) = UIMgr.realScreenSize
        let xInter = (UIMgr.instance.standardfontSize * 0.8)*(height/heightReal)
        return xInter
    }
    ...
    var divisionOperatorRect: CGRect {
        if operatorDivision != nil {
            ...
            return CGRectMake(0, self.frame.height/3.0, UIMgr.instance.standardfontSize*2.5, UIMgr.instance.standardfontSize*2.5)
        }
    }
    ...
    @objc func operatorACAction(_ sender: UIButton) {
        timeStampLatestClick = TimeMgr.currentTimeStamp
